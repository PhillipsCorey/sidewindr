#' Pull Top Rows of a Dataframe
#'
#' @description
#' `slice.head` takes two parameters, a dataframe `data` and a number `n`. It will attempt to slice `n` rows out of the dataframe from the top of the dataframe and return these sliced rows as a new dataframe. If `data` is grouped, it will slice `n` rows for each group in `data`.
#'
...
#' @examples
#' mtcars |> slice.head(5)
#' # slice the first 5 rows of mtcars
#' 
#' mtcars |> group.by(cyl) |> slice.head(2)
#' # slice the first 2 rows for each amount of cyl in the dataset
#' 
#' mtcars |> group.by(cyl) |> slice.head(15)
#' # notice: 15 is larger than the size of our groups, so some group slices may include the entire group!
#' @export
slice.head <- function(data, n){
  df = data
  rn = as.double(n)
